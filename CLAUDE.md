...
Use when building, testing, or deploying JavaScript/TypeScript applications. Reach for Bun when you need to run scripts, manage packages, bundle code, or test applications with a single unified toolkit.
...
Use feature-based folder organization. Group code by business domain/feature, not by technical layer.
...
│   │   ├── api/            # Auth API calls/types
│   │   ├── types.ts        # Auth domain types
│   │   └── index.ts        # Public exports (barrel file for feature)
│   ├── dashboard/
│   │   ├── components/
│   │   ├── hooks/
│   │   ├── api/
│   │   ├── types.ts
│   │   └── index.ts
│   └── [feature-name]/     # Each feature self-contained
├── shared/
│   ├── components/         # Truly shared UI (Button, Input primitives)
│   ├── hooks/              # Cross-feature hooks (useTheme, useMediaQuery)
│   ├── lib/                # Utilities (utils.ts, formatters)
│   └── types/              # Global types
├── routes/                 # TanStack Router route files only
...
- **Features are self-contained**: Each feature folder contains all its components, hooks, API logic, and types
- **Shared folder for cross-cutting code**: Only truly reusable primitives go in `shared/`
- **Routes stay in `routes/`**: Route files remain in TanStack Router's convention, but import from features
- **Barrel exports**: Each feature exports via `index.ts` for clean imports: `import { LoginForm } from '#/features/auth'`
- **No deep imports**: Import from feature root, not internal paths: `#/features/auth` not `#/features/auth/components/LoginForm`
- **UI primitives in shared/components/ui**: shadcn components belong in `shared/components/ui/`

### When to Create a Feature

- Business domain with multiple related components/hooks
- Has its own data types and API interactions
- Could potentially be extracted as a standalone module
